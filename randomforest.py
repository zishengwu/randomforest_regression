...
class Tree(object):
    ...
    def calc_predict_result(self, dataset):
        "使用决策树分类或回归"
    ...
    def describe_tree(self):
        ...
        tree_structure = "{split_feature:" + str(self.split_feature) +\
            ...
            ",left_tree:" + left_info + ",right_tree:" + right_info + "}"
        return tree_structure

class RandomForestRegereeion(object):
    def __init__(self, tree_nums=10, max_depth=-1, min_samples_split=2, min_samples_leaf=1,
        ...
        """
               ...
               subsample:         行采样比例
               random_state:      随机种子，设置之后每次生成的tree_nums:个样本集不会变，确保实验可重复
               """
    ...
    def train_model(self, dataset, targets):
        """训练模型"""
        targets = targets.to_frame(name='label') # Convert Series to DataFrame 将回归的目标变成这种格式，并且有头

        if self.random_state:
            random.seed(self.random_state)
        """单独使用random()函数所产生的随机数，在每一次程序重新启动后，总是重复同一组随机数字。
        如果希望程序重新启动后产生的随机数值与上一次程序运行时的随机数不同，则需要使用randomSeed()函数。"""

        random_state_stages = random.sample(range(self.tree_nums), self.tree_nums) # 随机行采样
        ...
        for stage in range(self.tree_nums):
            ...
            tree = self.fit_tree(dataset_copy, targets_copy, depth=0)
            self.trees[stage] = tree #字典的键值分别是 树的编号和树本身
            print(tree.describe_tree())

    def fit_tree(self, dataset, targets, depth):
        """递归建立决策树"""
        ...
        if depth < self.max_depth:
            ...
            if left_dataset.__len__() <= self.min_samples_leaf or \
                ...
                tree.tree_left = self.fit_tree(left_dataset, left_targets, depth + 1)
                tree.tree_right = self.fit_tree(right_dataset, right_targets, depth + 1)
                return tree
            # 如果树的深度超过预设值，则终止分裂
        else:
            tree = Tree()
    ...
    def choose_best_feature(self, dataset, targets):
        ...
        for feature in dataset.columns:
            ...
            for split_value in unique_values:
                ...
                if split_gain < best_split_gain: #在这里是MSE
